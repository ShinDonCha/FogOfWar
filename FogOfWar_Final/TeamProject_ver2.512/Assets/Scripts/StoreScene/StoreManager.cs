...
public class StoreManager : MonoBehaviour
{
    ...
    public void SetInfo(TowerInfo info)
    {
        ...
        infoStatText.text += "공격속도 : " + info.atkCycle + "\n";
    ...
    {
        infoNameText.text = Enum.GetDescription(type);
        ...
        switch(type)
        {
            case SkillType.Plane:
                infoStatText.text = "다음 레벨 효과 : " +
                    (20 + ((GlobalValue.SkillLvDic[SkillType.Plane] + 1) * 5)).ToString() +
                    "의 공격력으로 폭격";
                infoHelpText.text = "폭격기를 보내 타워를 공격하는 스킬입니다";
                break;
            case SkillType.Shield:
                infoStatText.text = "다음 레벨 효과 : 3초간 " +
                    ((1 + ((GlobalValue.SkillLvDic[SkillType.Shield] + 1) * .1f)) * 15).ToString() +
                    "의 범위에 보호막 설치";
